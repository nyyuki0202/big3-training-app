...
export default function SquatPage() {
    ...
    const fetchLastRecord = async () => {
      try {
        ...
        const { data } = await supabase
          .from('workouts').select('weight, reps')
          .eq('exercise', 'squat').eq('user_id', session.user.id)
  ...
  const handleRecord = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      await supabase.from('workouts').insert([{ exercise: 'squat', weight, reps, notes: notes.trim() || null, user_id: session?.user.id }]);
      router.push("/");
    } catch (e) { alert("Error..."); setIsSubmitting(false); }
  };

  if (isLoading) return <div className="min-h-screen bg-gray-900 flex items-center justify-center text-blue-500 font-black italic">LOADING...</div>;
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white p-6 pb-32 flex flex-col items-center">
      ...
      <h1 className="text-4xl font-black mb-10 text-blue-500 italic tracking-tighter drop-shadow-[0_0_15px_rgba(59,130,246,0.6)]">SQUAT</h1>
      ...
      <div className="w-full max-w-md space-y-8">
            ...
            <div className="flex-1 flex justify-center">
              {isWeightInputMode ? (
                <input type="number" inputMode="decimal" autoFocus onBlur={() => setIsWeightInputMode(false)}
                  ...
                  className="w-32 bg-transparent text-6xl font-black text-center text-blue-500 outline-none border-b-4 border-blue-600" />
              ) : (
                <div onClick={() => setIsWeightInputMode(true)} className="text-7xl font-black text-center text-blue-500 cursor-pointer drop-shadow-[0_0_25px_rgba(59,130,246,0.5)]">{weight}</div>
              )}
            </div>
            <button onClick={() => setWeight(w => w + 2.5)} className="w-16 h-16 bg-blue-600 rounded-full text-2xl font-black">+</button>
          </div>
        </div>
        ...
        <div className="bg-gray-800/80 p-8 rounded-[40px] border border-gray-700 shadow-2xl">
          ...
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="flex-1 text-7xl font-black text-center text-cyan-400 drop-shadow-[0_0_20px_rgba(34,211,238,0.5)]">{reps}</div>
            <button onClick={() => setReps(r => r + 1)} className="w-16 h-16 bg-cyan-600 rounded-full text-3xl font-bold">+</button>
          </div>
        </div>

        {/* --- 備考入力欄 --- */}
        <div className="w-full max-w-xs mb-8">
          <label className="text-[10px] text-gray-500 ml-4 mb-2 block uppercase tracking-widest italic font-bold">
            Optional_Notes
          </label>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="フォーム意識 / 立ち上がり重い..."
