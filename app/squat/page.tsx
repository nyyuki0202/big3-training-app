...
export default function SquatPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white p-6 flex flex-col items-center">
      ...
      <h1 className="text-3xl font-bold mb-8 text-blue-500 italic tracking-tighter">SQUAT</h1>
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="bg-gray-800 p-6 rounded-3xl shadow-2xl border border-gray-700/50">
          ...
          <div className="flex items-center justify-between mb-8 gap-2">
            ...
            <div className="flex-1 px-2 text-5xl sm:text-6xl font-black font-mono text-center text-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.4)]">
              {weight}
            </div>
            <button
              onClick={() => setWeight(w => w + 2.5)}
              className="w-20 h-20 shrink-0 bg-blue-600 rounded-full text-xl font-black shadow-lg shadow-blue-900/40 active:scale-90 transition-transform"
            >
              +2.5
            </button>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setWeight(w => w + 10)} className="flex-1 h-10 bg-gray-700 rounded-xl text-xs font-bold active:bg-gray-600 text-blue-400">+10</button>
              <button onClick={() => setWeight(w => w + 5)} className="flex-1 h-10 bg-gray-700 rounded-xl text-xs font-bold active:bg-gray-600 text-blue-400">+5</button>
              <button onClick={() => setWeight(w => w + 1)} className="flex-1 h-10 bg-gray-700 rounded-xl text-xs font-bold active:bg-gray-600 text-blue-400">+1</button>
            </div>
          </div>
        </div>
